...
export function GameCard({ game }: GameCardProps) {
  return (
    <Link href={`/games/${game.id}`}>
      <Card className="group overflow-hidden rounded-3xl border-2 border-border/50 bg-card hover:border-primary/50 hover:shadow-xl hover:shadow-primary/10 transition-all duration-300 cursor-pointer h-full flex flex-col">
        ...
        <CardFooter className="p-5 pt-0 mt-auto">
          <div className="w-full rounded-xl bg-secondary/10 py-3 text-center font-bold text-secondary transition-colors group-hover:bg-secondary group-hover:text-secondary-foreground">
            Joacă acum
          </div>
        </CardFooter>
      </Card>
